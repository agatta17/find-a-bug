...
const initGame = () => {
  ...
  levels.forEach((level) =>
    level.addEventListener("click", (event) => {
      levels.forEach((level) =>
        level.classList.remove("levels-list__item_selected")
      );
...
const createCards = () => {
  ...
  for (let i = 0; i < numberOfRows; i++) {
    ...
    for (let i = 0; i < numberOfCardsinRow; i++) {
      if (idOfCard === cardNumberWithBug)
        idOfCard = createCard(row, "bug-card", idOfCard);
